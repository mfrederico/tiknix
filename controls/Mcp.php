...
class Mcp extends BaseControls\Control {
    ...
    /* The hand-written tool array that stood here (@deprecated since ToolLoader, and still
       listing the removed workbench task tools) is gone; index() and config() ask the
       loader, which is the one source of truth for what this gateway serves. */
    ...
    public function index($params = null): void {
        $this->render('mcp/index', [
            'title' => 'MCP Server',
            'tools' => $this->toolLoader ? $this->toolLoader->getDefinitions() : [],
    ...
    public function config($params = null): void {
        ...
        if ($hasAuth && $this->authMember) {
            ...
            $response['_meta'] = [
                ...
                'total_tools' => count($this->toolLoader ? $this->toolLoader->getDefinitions() : []) + array_sum(array_column($accessibleServers, 'tool_count')),
                'instructions' => [
